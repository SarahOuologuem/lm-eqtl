...
def model_eval(
    model, optimizer, dataloader, device, diploid=False, get_embeddings=False, temperature=None, silent=False
):
    criterion = torch.nn.CrossEntropyLoss(reduction="mean")
    ...
    with torch.no_grad():
        for itr_idx, ((masked_sequence, species_label), targets_masked, targets, seq) in enumerate(dataloader):
            if not diploid:  # predict haplotypes
                if get_embeddings:
            ...
            else:  # predict genotypes
                # NOTE: we expect batch_size = 1 here!
                if get_embeddings:
                    # masked sequence
                    masked_sequence1 = masked_sequence[0]
                    masked_sequence2 = masked_sequence[1]
                    # targets_masked
                    targets_masked1 = targets_masked[0]
                    targets_masked2 = targets_masked[1]
                    # targets
                    targets1 = targets[0]
                    targets2 = targets[1]
                    ...
                    # masked sequence
                    masked_sequence1 = torch.split(masked_sequence, split_size_or_sections=1, dim=0)[0]
                    masked_sequence2 = torch.split(masked_sequence, split_size_or_sections=1, dim=0)[1]
                    # targets_masked
                    targets_masked1 = torch.split(targets_masked, split_size_or_sections=1, dim=0)[0]
                    targets_masked2 = torch.split(targets_masked, split_size_or_sections=1, dim=0)[1]
                    # targets
                    targets1 = torch.split(targets, split_size_or_sections=1, dim=0)[0]
                    targets2 = torch.split(targets, split_size_or_sections=1, dim=0)[1]
                    species_label = species_label[0]
                    species_label = species_label.tile((len(masked_sequence1),))
                ...
                if get_embeddings:
                    ...
                    sequence_embedding2 = sequence_embedding2.mean(dim=0)
                    ...
                    all_embeddings.append((f"{seq_name}_father", sequence_embedding1, ground_truth_logprobs1))
                    all_embeddings.append((f"{seq_name}_mother", sequence_embedding2, ground_truth_logprobs2))
