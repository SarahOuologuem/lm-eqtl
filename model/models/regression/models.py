...
class DenseRegDSSResNet(nn.Module):
    ...
    def __init__(
        ...
        self.aggregate_haplotypes = aggregate_haplotypes
        self.emb_aggregation = emb_aggregation
    ...
    def forward(self, x, xs):
        ...
        # if the model is trained on genotypes, then the embeddings of the parents' haplotypes
        # are concatenated and aggregated
        if not self.haplotypes: 
            N, D, L = seq_embedding.shape # Obs, Embedding dim, Sequence length
            # aggregate the haplotype embeddings -> generates one embedding of dim d_model
            if self.aggregate_haplotypes:
                seq_embedding = seq_embedding.reshape(N // 2, 2, D, L)
                if self.emb_aggregation == "mean": 
                    seq_embedding = seq_embedding.mean(dim=1)
                else: 
                    seq_embedding = seq_embedding.sum(dim=1)
            # concatenate the haplotype embeddings -> generates one embedding of dim 2*d_model
            else: 
                seq_embedding = seq_embedding.reshape(N // 2, D * 2, L)

        
        # aggregate out the sequence length dimension
        ...
        return x, aggregated_embs, reg_y
